...
const buttonVariants = cva(
  // `transition-all duration-200 ease-out`, not the previous `transition-colors`:
  // every button on the site shares this one definition, so "smoother
  // hover/active transitions" (Almedin's ask) is one change here rather than
  // a className patched onto each call site. Deliberately no transform/scale
  // — this site's restraint (DESIGN.md §6) asked for smoother, not more
  // motion.
